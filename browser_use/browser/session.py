...
_GLOB_WARNING_SHOWN = (
    False  # used inside _is_url_allowed to avoid spamming the logs with the same warning multiple times
)
...
class BrowserSession(BaseModel):
    ...
    _original_browser_session: Any = PrivateAttr(
        default=None
    )  # Reference to prevent GC of the original session when copied
    _owns_browser_resources: bool = PrivateAttr(
        default=True
    )  # True if this instance owns and should clean up browser resources
    ...
    def _connection_str(self) -> str:
        ...
        binary_name = (
            ...
            else (self.browser_profile.channel or BROWSERUSE_DEFAULT_CHANNEL)
            .name.lower()
            .replace("_", "-")
            .replace(" ", "-")
    ...
    async def stop(self, _hint: str = "") -> None:
        ...
        if not self._owns_browser_resources:
            self.logger.debug(
                f"🔗 BrowserSession.stop() called on a copy, not closing shared browser resources {_hint}"
            )
        ...
        if self.browser_context or self.browser:
            self.logger.info(
                f"🛑 Closing {self._connection_str} browser context {_hint} {self.browser or self.browser_context}"
            )
    ...
    async def _unsafe_get_or_start_playwright_object(self) -> PlaywrightOrPatchright:
        ...
        if global_api_object and not should_recreate:
            try:
                ...
                self.logger.debug(
                    f"Detected invalid {driver_name} instance: {type(e).__name__}. Creating new instance."
                )
                should_recreate = True
    ...
    async def _unsafe_terminate_browser_process(self, _hint: str = "") -> None:
        ...
        if self.browser_pid:
            try:
                ...
                self.logger.info(
                    f" ↳ Killing browser_pid={self.browser_pid} {_log_pretty_path(executable_path)} {_hint}"
                )
    ...
    async def setup_playwright(self) -> None:
        ...
        if is_stealth:
            ...
            self.logger.info(
                f"🕶️ Activated stealth mode using patchright {self.browser_profile.channel.name.lower()} browser..."
            )
        else:
        ...
        if is_stealth:
            ...
            if not self.browser_profile.user_data_dir:
                self.logger.info(
                    " 🪄 For maximum stealth, BrowserSession(...) should be passed a persistent user_data_dir=..."
                )
            if self.browser_profile.headless or not self.browser_profile.no_viewport:
                self.logger.info(
                    " 🪄 For maximum stealth, BrowserSession(...) should be passed headless=False & viewport=None"
                )
    ...
    async def setup_browser_via_browser_pid(self) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ Expected Chrome process with pid={self.browser_pid} not found, unable to (re-)connect"
            )
            return
        except Exception as e:
            self.browser_pid = None
            self.logger.warning(
                f"⚠️ Error accessing chrome process with pid={self.browser_pid}: {type(e).__name__}: {e}"
            )
            return

        # check that browser_pid process is exposing a debug port we can connect to, otherwise we cannot connect to it
        debug_port = (
            next((arg for arg in args if arg.startswith("--remote-debugging-port=")), "").split("=")[-1].strip()
        )
                    ...
                    if not chrome_process.is_running():
                        ...
                        if hasattr(self, "_subprocess") and self._subprocess:
                            ...
                            if "Failed parsing extensions" in stderr_output:
                                self.logger.error(
                                    f"❌ Chrome process {self.browser_pid} exited: Failed parsing extensions"
                                )
    ...
    async def setup_browser_via_cdp_url(self) -> None:
        ...
        self.logger.info(
            f"🌎 Connecting to existing chromium-based browser via CDP: {self.cdp_url} -> (remote browser)"
        )
        assert self.playwright is not None, "playwright instance is None"
        self.browser = self.browser or await self.playwright.chromium.connect_over_cdp(
            self.cdp_url,
    ...
    async def _unsafe_setup_new_browser_context(self) -> None:
        ...
        if self.browser and not self.browser_context:
            ...
            if (
                self.browser_profile.record_har_path or self.browser_profile.record_video_dir
            ) and self.browser.contexts:
                recording_types = []
                if self.browser_profile.record_har_path:
                ...
                if hasattr(self, "_subprocess") and self._subprocess and self._subprocess.pid == self.browser_pid:
                    self.logger.debug(
                        f"👤 Using default browser_context opened in newly spawned browser: {self.browser_context}"
                    )
                else:
                    self.logger.info(
                        f"👤 Using first browser_context found in existing browser: {self.browser_context}"
                    )
            else:
                self.browser_context = await self.browser.new_context(
                    **self.browser_profile.kwargs_for_new_context().model_dump(mode="json")
        ...
        if not self.browser_context:
            ...
            try:
                ...
                self.playwright = await self._start_global_playwright_subprocess(
                    is_stealth=self.browser_profile.stealth
                )
    ...
    async def _setup_viewports(self) -> None:
        ...
        assert (
            self.browser_context
        ), "BrowserSession.browser_context must already be set up before calling _setup_viewports()"
                ...
                if self.browser_profile.downloads_path:
                    try:
                        ...
                        unique_filename = await self._get_unique_filename(
                            self.browser_profile.downloads_path, suggested_filename
                        )
                        ...
                        self.logger.info(
                            f"📁 Added download to session tracking (total: {len(self._downloaded_files)} files)"
                        )
                        ...
                        if element_node.viewport_coordinates and element_node.viewport_coordinates.center:
                            try:
                                ...
                                await page.mouse.click(
                                    element_node.viewport_coordinates.center.x,
                                    element_node.viewport_coordinates.center.y,
                                )
                                try:
                                    await page.wait_for_load_state()
                                except Exception:
    ...
    async def navigate(self, url: str = "about:blank", new_tab: bool = False, timeout_ms: int | None = None) -> Page:
        ...
        try:
            ...
            self.logger.debug(
                f"🧭 Starting navigation to {_log_pretty_url(normalized_url)} with timeout {timeout_ms}ms"
            )
    ...
    async def _save_storage_state_to_file(self, path: str | Path, storage_state: dict[str, Any] | None) -> None:
        try:
            ...
            self.logger.warning(
                f"❌ Failed to save cookies to storage_state= {_log_pretty_path(path)}: {type(e).__name__}: {e}"
            )
    ...
    async def load_storage_state(self) -> None:
        ...
        if self.browser_profile.cookies_file:
            ...
            if not cookies_path.is_absolute():
                cookies_path = (
                    Path(self.browser_profile.downloads_path or ".").expanduser().resolve() / cookies_path.name
                )
            ...
            try:
                ...
                if cookies_data:
                    ...
                    self.logger.info(
                        f"🍪 Loaded {len(cookies_data)} cookies from cookies_file= {_log_pretty_path(cookies_path)}"
                    )
            except Exception as e:
                self.logger.warning(
        ...
        if self.browser_profile.storage_state:
            ...
            try:
                ...
                self.logger.warning(
                    f"❌ Failed to load cookies from storage_state= {storage_state}: {type(e).__name__}: {e}"
                )
                return
    ...
    async def wait_for_page_and_frames_load(self, timeout_overwrite: float | None = None):
        ...
        if bytes_used is not None:
            ...
            self.logger.info(
                f"➡️ Page navigation [{tab_idx}]{_log_pretty_url(page.url, 40)} took {elapsed:.2f}s{extra_delay}"
            )
    ...
    async def refresh_page(self):
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ Page {_log_pretty_url(page.url)} failed to fully load after refresh: {type(e).__name__}: {e}"
            )
    ...
    async def get_state_summary(self, cache_clickable_elements_hashes: bool) -> BrowserStateSummary:
		...
		"""
        await self.wait_for_page_and_frames_load()
        updated_state = await self._get_updated_state()
        if len(updated_state.selector_map) == 0:
            self.logger.warning(
                f"No interactive elements found on {updated_state.url}. Waiting 3 seconds and retrying..."
            )
            ...
            try:
                ...
                if blocked_target_id:
                    ...
                    await asyncio.wait_for(
                        cdp_session.send("Target.closeTarget", {"targetId": blocked_target_id}), timeout=2.0
                    )
    ...
    async def take_screenshot(self, full_page: bool = False) -> str | None:
        ...
        try:
            ...
            if "timeout" in error_str.lower():
                self.logger.warning(
                    f"⏱️ Screenshot timed out on page {_log_pretty_url(page.url)} (possibly crashed): {error_str}"
                )
            else:
                self.logger.error(
                    f"❌ Screenshot failed on page {_log_pretty_url(page.url)} (possibly crashed): {error_str}"
                )
            raise
        finally:
            if cdp_session:
    ...
    async def get_locate_element_by_xpath(self, xpath: str) -> ElementHandle | None:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Failed to locate xpath {xpath} on page {_log_pretty_url(page.url)}: {type(e).__name__}: {e}"
            )
            return None
    ...
    async def get_locate_element_by_text(
        ...
        try:
            ...
            if not elements:
                self.logger.error(
                    f"❌ No visible element with text '{text}' found on page {_log_pretty_url(page.url)}."
                )
                return None
        ...
        try:
            ...
            try:
                ...
                if download_result and download_result.get("data") and len(download_result["data"]) > 0:
                    ...
                    self.logger.info(
                        f"📄 Auto-downloaded PDF ({cache_status}, {response_size:,} bytes): {download_path}"
                    )
