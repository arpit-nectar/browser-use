...
class SystemPrompt:
    ...
    def _load_prompt_template(self) -> None:
        ...
        try:
            ...
            with (
                importlib.resources.files("browser_use.agent")
...
class AgentMessagePrompt:
    def __init__(
        ...
        previous_browser_state: "BrowserStateSummary | None" = None,
    ):
        ...
        self.previous_browser_state = previous_browser_state
    ...
    def _deduplicate_screenshots(self, screenshots: list[str]) -> list[str]:
        ...
        for i in range(len(screenshots)):
            ...
            if i == len(screenshots) - 1 or screenshots[i] != screenshots[i + 1]:
...
"""
    ...
    def _get_previous_browser_state_description(self, previous_state: "BrowserStateSummary | None") -> str:
        """Build the previous browser state description showing previous URLs for each tab"""
        if not previous_state:
            return "No previous browser state available (this is the first step)"

        # Build the previous tab information
        previous_tabs_text = ""
        for tab in previous_state.tabs:
            previous_tabs_text += f"Tab {tab.page_id}: {tab.url} - {tab.title[:30]}\n"

        previous_browser_state = f"""Previous URLs for each tab before last actions:
{previous_tabs_text}"""
        return previous_browser_state

    def _get_agent_state_description(self) -> str:
        if self.step_info:
            step_info_description = (
                f"Step {self.step_info.step_number + 1} of {self.step_info.max_steps} max possible steps\n"
            )
        else:
            step_info_description = ""
...
"""
    ...
    def get_user_message(self, use_vision: bool = True) -> UserMessage:
        ...
        state_description += (
            ...
            + self._get_previous_browser_state_description(self.previous_browser_state).strip("\n")
            + "\n</previous_browser_state>\n"
        )
        state_description += (
