...
from browser_use.observability import is_debug_mode, observe, observe_debug
...
class Agent(Generic[Context, AgentStructuredOutput]):
    ...
    def __init__(
        ...
        save_screenshots_in_debug: bool = True,
        max_failures: int = 3,
        retry_delay: int = 10,
        ...
        **kwargs,
        ...
        self.settings = AgentSettings(
            ...
            save_screenshots_in_debug=save_screenshots_in_debug,
    ...
    def logger(self) -> logging.Logger:
        ...
        return logging.getLogger(
            f"browser_use.Agent🅰 {self.task_id[-4:]} on 🆂 {_browser_session_id[-4:]} 🅟 {_current_page_id}"
        )
    ...
    async def _get_model_output_with_retry(self, input_messages: list[BaseMessage]) -> AgentOutput:
        ...
        if (
            ...
            if not model_output.action or all(action.model_dump() == {} for action in model_output.action):
                ...
                action_instance.done = {"success": False, "text": "No next action returned by LLM!"}
    ...
    async def _handle_post_llm_processing(
        ...
        if self.register_new_step_callback and self.state.last_model_output:
            if inspect.iscoroutinefunction(self.register_new_step_callback):
                await self.register_new_step_callback(
                    browser_state_summary, self.state.last_model_output, self.state.n_steps
                )
            else:
        ...
        if self.settings.save_conversation_path and self.state.last_model_output:
            ...
            # Check if we should save screenshots
            should_save_screenshots = self.settings.save_screenshots or (
                self.settings.save_screenshots_in_debug and is_debug_mode()
            )

            if should_save_screenshots and browser_state_summary:
                # Save conversation with screenshots using the screenshot manager
                conversation_path, screenshot_path = await save_conversation_with_screenshots(
                    input_messages,
                ...
                # Save conversation only (original behavior)
    ...
    def _log_step_completion_summary(self, step_start_time: float, result: list[ActionResult]) -> None:
        ...
        self.logger.info(
            f"📍 Step {self.state.n_steps}: Ran {action_count} actions in {step_duration:.2f}s: {status_str}"
        )
            ...
            for step in range(max_steps):
                ...
                if on_step_end is not None:
                    if self.browser_session:
                        await self.browser_session.wait_for_page_and_frames_load()
