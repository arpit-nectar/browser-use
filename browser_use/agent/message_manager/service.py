...
class MessageManager:
    ...
    def add_state_message(
        ...
        if agent_history_list and self.images_per_step > 1:
            ...
            raw_screenshots = agent_history_list.screenshots(
        ...
        # Extract previous browser state from agent history (if available)
        previous_browser_state = None
        if agent_history_list and len(agent_history_list.history) > 0:
            # Get the most recent history item which contains the previous browser state
            last_history_item = agent_history_list.history[-1]
            # Convert BrowserStateHistory to BrowserStateSummary for consistency
            if hasattr(last_history_item.state, "url"):
                # Create a minimal BrowserStateSummary from the previous state
                # We only need URL and tabs info for the previous browser state component
                from browser_use.browser.views import BrowserStateSummary
                from browser_use.dom.views import DOMElementNode

                # Create a minimal DOM element tree (not used in previous state display)
                ...
                previous_browser_state = BrowserStateSummary(
                    element_tree=minimal_element_tree,
                    selector_map={},
                    url=last_history_item.state.url,
                    title=last_history_item.state.title,
                    tabs=last_history_item.state.tabs,
                    screenshot=None,  # Don't need screenshot for previous state
                    pixels_above=0,
                    pixels_below=0,
                    browser_errors=[],
                )
    ...
    def _add_message_with_type(
        self, message: BaseMessage, message_type: Literal["system", "state", "consistent"]
    ) -> None:
